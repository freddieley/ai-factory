...
- [x] Power, thermal, signal-integrity, and interface analysis — deterministic power budget, current draw, thermal dissipation/resistance/temperature-rise, operating-temperature-limit, signal frequency/impedance/termination, protocol/interface coverage, and logic-voltage-compatibility checks against the requirements-driven architecture, with persisted hash-verified analysis artifacts, capability discovery, API execution, and unit/API regression coverage. Verified against real GitHub Actions CI (`CI` and `quality` workflows, including the KiCad-integration job) green on `main`.
