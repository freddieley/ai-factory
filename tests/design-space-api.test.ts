...
import { registerDesignSpaceRoutes } from "../src/design-space-routes.js";
...
describe("design-space exploration factory surface", () => {
  ...
  it("persists an auditable exploration artifact and returns the ranked candidates", async () => {
    ...
    registerDesignSpaceRoutes(testApp);
    await testApp.ready();
    try {
      ...
      const list = await testApp.inject({ method: "GET", url: `/api/projects/${project.id}/design-space/explorations` });
