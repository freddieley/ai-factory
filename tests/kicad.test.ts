...
const ok = (args: string[], stdout = "KiCad 10.0.0"): KiCadCommandResult => ({ command: ["kicad-cli", ...args], exitCode: 0, stdout, stderr: "" });
...
describe("KiCad integration adapter", () => {
  ...
  it("constructs real ERC, DRC, and netlist commands without shell interpolation", async () => {
    ...
    try {
      const result = await validateKiCadDesign({
        workspaceRoot: root,
        schematicPath: "project/design.kicad_sch",
        pcbPath: "project/design.kicad_pcb",
        exportNetlist: true,
        outputDirectory: join(root, "validation"),
        runner: async args => {
          calls.push(args);
          if (args[0] === "version") return ok(args);
          return ok(args);
        },
      });
  ...
  it.skipIf(process.env.KICAD_CLI_INTEGRATION !== "1")("runs against an installed KiCad CLI", async () => {
    ...
    const result = await validateKiCadDesign({
      workspaceRoot: root,
      schematicPath: "fixtures/minimal.kicad_sch",
      pcbPath: "fixtures/minimal.kicad_pcb",
      exportNetlist: true,
    });
