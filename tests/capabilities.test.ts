...
describe("factory capability registry", () => {
  it("keeps deterministic capabilities uniquely named and exposes OpenAI-compatible tools", () => {
    ...
    const tools = toOpenAITools(); expect(tools).toHaveLength(capabilities.length); expect(tools.map(tool => tool.type)).toEqual(names.map(() => "function")); expect(tools.map(tool => tool.function.name)).toEqual(names);
  });
