...
describe("factory capability registry", () => {
  ...
  it("exposes the Fusion document inspection operation with queryType as the only required discriminator", () => { const tool=getCapability("ai_factory_inspect_fusion"); expect(tool?.parameters).toMatchObject({ type:"object", properties:{ queryType:{type:"string",enum:["document","projects","apiDocumentation","screenshot","activeCommand"]} }, required:["queryType"] }); });
