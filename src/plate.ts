...
export function createPlateWithHoleScript(args: CreatePlateArgs): string {
  ...
  const x = (args.holeXmm ?? args.widthMm / 2) / 10;
  const y = (args.holeYmm ?? args.depthMm / 2) / 10;
  const r = args.holeDiameterMm / 20;
...
# Through-hole sketch on the top face. The sketch is attached to the face,
# so the local origin is the lower-left corner of the plate.
...
function parseResult(result: unknown): PlateResult {
  ...
  const diameter = text.match(/hole_diameter_mm=([0-9.eE+-]+)/)?.[1];
  const x = text.match(/hole_x_mm=([0-9.eE+-]+)/)?.[1];
  const y = text.match(/hole_y_mm=([0-9.eE+-]+)/)?.[1];
...
export async function executeCreatePlate(args: Record<string, unknown>): Promise<PlateResult> {
  ...
  const result = await withTimeout(
    fusion.callTool("fusion_mcp_execute", { featureType: "script", object: { script: createPlateWithHoleScript(parsed) } }),
    config.TOOL_TIMEOUT_MS,
    "Fusion create plate with hole"
  );
