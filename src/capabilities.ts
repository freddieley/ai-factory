...
export const capabilities: Capability[] = [
  ...
  { name: "ai_factory_inspect_fusion", domain: "cad", description: "Inspect the current Fusion document using a bounded read-only operation. Use this once when existing CAD state matters before deciding whether changes are needed.", parameters: { type: "object", properties: { operation: { type: "string", enum: ["activeDocument"] } }, required: ["operation"] }, execute: async (args) => { const operation = String(args.operation); if (operation !== "activeDocument") throw new Error("Unsupported Fusion inspection operation."); return fusion.callTool("fusion_mcp_read", { operation }); } },
