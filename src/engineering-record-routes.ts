...
import { z } from "zod";
import { getProject, listArtifactRevisions } from "./db.js";
...
export async function registerEngineeringRecordRoutes(app:FastifyInstance){
  ...
  app.get("/api/artifacts/:id/revisions",async(request,reply)=>{const {id}=request.params as {id:string};if(!listArtifactRevisions(id).length)return reply.code(404).send({error:"artifact not found or has no revisions"});return listArtifactRevisions(id);});
}
