...
import { runFirmwareHil } from "./firmware-io.js";
...
export function registerFirmwareRoutes(app: FastifyInstance): void {
  ...
  app.post("/api/projects/:id/firmware/hil", async (request, reply) => {
    ...
    try {
      ...
      if (!body?.project) return reply.code(400).send({ error: "project is required" });
      const result = await runFirmwareHil(body.project);
      const contentHash = createHash("sha256").update(JSON.stringify(result)).digest("hex");
      const artifactId = createArtifact(id, undefined, "firmware_hil", "Firmware host HIL result", undefined, contentHash, { schema: result.schema, status: result.status, eventCount: result.events.length });
      return { artifactId, contentHash, result };
    } catch (error) { return reply.code(400).send({ error: String(error) }); }
  });
