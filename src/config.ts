...
const schema = z.object({
  ...
  MAX_MODEL_CALLS: z.coerce.number().int().min(1).max(20).default(6),
  ...
  MODEL_TIMEOUT_MS: z.coerce.number().int().min(1_000).max(120_000).default(45_000),
  MODEL_RETRIES: z.coerce.number().int().min(0).max(3).default(1),
