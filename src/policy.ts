...
export function evaluateFusionOperation(toolName: string, args: Record<string, unknown>): PolicyDecision {
  ...
  if (CAD_COMPILE_CAPABILITIES.has(toolName)) return { allowed:true, requiresApproval:false, operationClass:"write", reason:"Validated model-authored CAD is being compiled into the connected Fusion document; this is CAD authoring, not physical machine execution." };
