...
function mcpToolsAsOpenAI(): OpenAI.Chat.Completions.ChatCompletionTool[] {
  return fusion.getTools().map(tool => ({ type: "function", function: {
  ...
  try {
    ...
    for (let step = 1; step <= config.MAX_AGENT_STEPS; step++) {
      ...
      for (const call of calls) {
        ...
        const toolName = rawName.startsWith("fusion__") ? rawName.slice("fusion__".length) : rawName;
        ...
        if (!policy.allowed) {
          messages.push({ role: "tool", tool_call_id: call.id, content: JSON.stringify({ error: policy.reason }) });
          continue;
        }

        if (policy.requiresApproval) {
          const approvalId = requestApproval(projectId, toolName, { runId, toolName, args, reason: policy.reason });
          addEvent(runId, "approval.requested", { approvalId, toolName, args, reason: policy.reason });
          messages.push({ role: "tool", tool_call_id: call.id, content: JSON.stringify({ status: "blocked_pending_approval", approvalId, reason: policy.reason }) });
          continue;
        }
        ...
        try {
          ...
          messages.push({ role: "tool", tool_call_id: call.id, content: JSON.stringify({ error: String(error), toolName }) });
        }
      }
    }
